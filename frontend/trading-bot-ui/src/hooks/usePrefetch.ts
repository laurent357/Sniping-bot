...
export const usePrefetch = (dataFetchers: DataFetcher, options: PrefetchOptions = {}) => {
  ...
  const prefetchRoute = useCallback(
    async (path: string) => {
      ...
      const scheduleTask =
        (window as any).requestIdleCallback || ((cb: Function) => setTimeout(cb, delay));

      scheduleTask(async () => {
        console.debug(`Starting prefetch for route: ${path}`);
        const startTime = performance.now();

        await Promise.all(relevantData.map(dataKey => prefetchData(dataKey)));
